...
| `data-fui-rpc-with="<module[,module]>"` | Before dispatching, the runtime loads each named module (`loadModule`, idempotent) and only then runs the request hooks on `window.__gofastr._rpcHooks.request` — `async (node, req)` over one `{path, method, body, isFormData, headers}` object the fetch is built from — so a module that must ride on the request is registered before the fetch instead of racing the marker scan. On a 2xx the response hooks (`_rpcHooks.response`, `(node, response)`) run after the runtime's own headers (invalidate, push-state, toast) and before the body is applied. The seam exists for `framework/local`'s upload and download bridges (`data-local-send` and the `X-Gofastr-Local` header); a module that fails to load is logged and the dispatch proceeds without it. |

...
| `data-fui-rpc-with="<module[,module]>"` | Before dispatching, the runtime loads each named module (`loadModule`, idempotent) and only then runs the request hooks on `window.__gofastr._rpcHooks.request` — `async (node, req)` over one `{path, method, body, isFormData, headers}` object the fetch is built from — so a module that must ride on the request is registered before the fetch instead of racing the marker scan. On a 2xx the response hooks (`_rpcHooks.response`, `(node, response)`) are awaited in order after the runtime's own headers (invalidate, push-state, toast) and before the body is applied; a response hook must not read the body, because the runtime reads it once after the hooks, and a hook that rejects is logged without cancelling the response. The seam exists for `framework/local`'s upload and download bridges (`data-local-send` and the `X-Gofastr-Local` header). It FAILS CLOSED: naming a module here declares it a precondition of the request, so a module that will not load, a hook that throws, or a hook that sets `req.fatal` cancels the dispatch and raises `gofastr:rpc-refused` on `window`. Dispatching anyway would hand the server a request that looks complete and is not, with nothing in the page to say so. |
...
links do for modals. Because the server owns first paint, read the
...
- **Browser-persisted.** `Slice.Persist()` / `.PersistMax(n)` marks a
  slice the browser remembers: every binding it renders carries
  `data-fui-signal-persist="<cap>"`, the `signal-persist` behaviour
  (`core-ui/store/persist.js`, `Requires("local")`) restores the
  browser's value into the signal after hydration, writes every later
  value back through the `local` primitive above, and mirrors another
...
behaviour seam as three modules, `local-store` with `Requires("local")`,
`local-bridge` and `local-migrate` with `Requires("local-store")` — one
file per responsibility, each under the per-module byte budget that
holds registered behaviours too — owning their own `data-local-*` markers and reaching this store only
...
11. **Async runtime modules mark the trigger `aria-busy="true"` during in-flight RPCs and ignore re-entry.** Without it, keyboard Enter/Space fires duplicate submits and screen readers don't announce the state change. Clear it on commit / idle. Do not reach for `disabled` to get the re-entry guard: disabling a focused button drops keyboard focus to the body, and `disabled` has other owners (a hidden conditional region disables its controls) whose decision a settlement must not undo; the kernel's `action` primitive is the reference (a click while `pending` is a no-op, `disabled` untouched).

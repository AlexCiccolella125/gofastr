...
| `data-fui-rpc-with="<module[,module]>"` | The runtime loads each named module before dispatching and runs the registered request hooks (`__gofastr._rpcHooks.request`) over the request object the fetch is built from; on a 2xx the response hooks are awaited in order after the runtime's own headers and before the body is read, so a response hook must not read the body (the runtime reads it once, after the hooks) and a hook that rejects is logged, not fatal. The seam `framework/local`'s upload (`data-local-send`) and download (`X-Gofastr-Local`) bridges ride on. It fails closed: a module that will not load, or a hook that throws or marks `req.fatal`, cancels the dispatch and raises `gofastr:rpc-refused` — naming a module here declares it a precondition of the request, not a hint. See [Local state](local-state.md). |
...
| `data-fui-rpc-trigger="input"` | On a `data-fui-rpc` carrier (a `<form>` or any element — the combobox uses a `<div>` so a host form survives HTML parsing), dispatch the RPC on every `input` event from any control inside, after a debounce window. A non-form carrier's named, enabled controls are serialized into the body exactly like a form's. |
...
| `data-fui-rpc-scroll-to="<selector>"` | On 2xx RPC, smooth-scroll the matching element into view. Use to direct the user's eye at newly-inserted content. A malformed selector is a no-op; it can never corrupt the RPC response signal. |
...
| `data-fui-layout="<name>"` | Set by EVERY layout layer on its wrapper `<div>` with the layout's name (e.g. `app`, `marketing`). Emit-only since the layout-chain rewrite: it is the CSS/debug contract (`.layout-<name>` pairing); the runtime's swap decisions read `data-fui-layout-key` instead. |
...
| `data-fui-banner-dismiss-id="<id>"` | Optional companion to `data-fui-banner-dismiss`. When set, the dismissal is recorded in `localStorage` under `gofastr.banner-dismiss.<encodeURIComponent(id)>` (and mirrored into a cookie of the same name so `ui.Banner` can skip rendering server-side); the id is component-encoded at the storage boundary so a DOM-sourced value can never contribute cookie delimiters. For the usual letters-and-digits ids the key is the id verbatim. The same banner is auto-hidden on every subsequent page load until the key is cleared. Use for "deprecation notice — got it" banners. |
...
| `data-fui-sortable-rpc="<path>"` | POST endpoint that receives the commit after every successful reorder; non-2xx response reverts the DOM. Same-container reorders send `order=<comma-separated-keys>` plus `container=<id>` when the list carries `data-fui-sortable-container` (#84) and `version=<token>` when versioned. Cross-container drops add `moved=<key>` and always carry `container=` (empty if unconfigured). |
| `data-fui-sortable-item` | Marks an `<li>` as a drag-and-drop item inside a `data-fui-sortable` list. Pair with `data-fui-sort-key` and (typically) `draggable="true"` + `tabindex="0"`. Keyboard: Space grabs / drops; Arrow Up/Down moves while grabbed; Arrow Left/Right moves to an adjacent column (same group, including an empty one); Esc cancels. A `data-fui-sortable` list may legally hold zero items (empty Kanban column, #82) and stays a valid drop target. |
...
| `data-fui-sortable-container="<id>"` | Per-column id emitted on each `data-fui-sortable` `<ol>` of a linked board. Sent as the `container` body field in EVERY commit from that list; same-container reorders included (#84). Distinct from `data-fui-sortable-group` (the board id) because a board has one group but N containers; the server needs both to route the write. Lists without this attr keep the legacy payload (no `container` on same-container commits; empty `container=` on cross-container commits). |
